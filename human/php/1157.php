<?php

use phpseclib3\Crypt\RSA;
use phpseclib3\Crypt\RC4;
use phpseclib3\Crypt\Base64;

$private_key = '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----';

$encrypted = 'E+moZJOeVuxgsg==';
$envelope = '';

$rsa = new RSA();
$rsa->loadKey($private_key);
$theKey = $rsa->decrypt($envelope);

$rc4 = new RC4();
$rc4->setKey($theKey);
$decrypted = $rc4->decrypt(base64_decode($encrypted));

echo $decrypted;

?>