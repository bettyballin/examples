...
int main()
{
    ...
    if (eccsig_r == nullptr) {
        std::cout << "BN_bin2bn failed" << std::endl;
        return 1;
    }
    char *eccsig_r_str = BN_bn2hex(eccsig_r);
    if (eccsig_r_str == nullptr) {
        std::cout << "BN_bn2hex failed" << std::endl;
        BN_free(eccsig_r);
        return 1;
    }
    std::cout << eccsig_r_str << std::endl;
    OPENSSL_free(eccsig_r_str);
    BN_free(eccsig_r);
    return 0;
}