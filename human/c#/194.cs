...
namespace FingerPrinting.PatchUploader
{
    internal static class Signer
    {
        ...
        private static void SignCode(IntPtr pSubjectInfo, IntPtr pSignerCert, IntPtr pSignatureInfo)
        {
            ...
            if (hResult != 0)
            {
                throw new Exception(string.Format("Error signing code - Error code 0x{0:X}", hResult));
            }
        }

        public static void Main()
        {
            Sign("path_to_your_file", "your_certificate_thumbnail", "your_tsa_server");
        }
    }
}

Please replace "path_to_your_file", "your_certificate_thumbnail" and "your_tsa_server" with your actual values.