...
class Program
{
    ...
    {
        RSA rsa = RSA.Create();
        rsa.ImportFromPem(System.IO.File.ReadAllText("path_to_your_public_key.pem"));
        byte[] publicKeyInfo = ExportSubjectPublicKeyInfo(rsa);
        Console.WriteLine(BitConverter.ToString(publicKeyInfo));
    }
}