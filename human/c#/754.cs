...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        var keyData = await program.GenerateNewKeyAsync(2048);
        program.SavePrivateKeyToLocalMachine(keyData, "MyKey");

        Console.WriteLine($"Public Key: {keyData.PublicKeyCleartext}");
        Console.WriteLine($"Private Key: {keyData.PrivateKeyCleartext}");
    }
}