...
namespace FooTest
{
    class FooImplementing
    {
        ...
        static void Main()
        {
            ...
            FooImplementing foo = new FooImplementing("Hello World");

            byte[] blindedMessage = foo.blindTheMessage(t1);
            byte[] blindSignedMessage = foo.blindSignature(blindedMessage);
            byte[] unblindedMessage = foo.unblindeTheSignedData(blindSignedMessage);
            bool isValid = foo.verifyBlindSignature(unblindedMessage, t1);

            byte[] signedData = foo.signedWithRsa(foo.inputMessage);
            bool isValidRsa = foo.verifyRsaSignedData(foo.inputMessage, signedData);

            t1.Text += "\nBlinded Message: " + BitConverter.ToString(blindedMessage);
            t1.Text += "\nBlind Signed Message: " + BitConverter.ToString(blindSignedMessage);
            t1.Text += "\nUnblinded Message: " + BitConverter.ToString(unblindedMessage);
            t1.Text += "\nIs Valid: " + isValid.ToString();
            t1.Text += "\nSigned Data: " + BitConverter.ToString(signedData);
            t1.Text += "\nIs Valid RSA: " + isValidRsa.ToString();
