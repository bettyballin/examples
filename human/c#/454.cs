...
class Program
{
    ...
    {
        Credential credential = CredentialManager.ReadCredential("Test");
        if (credential != null)
        {
            Console.WriteLine(credential.ToString());
        }

        CredentialManager.WriteCredential("Test", "User", "Password");

        IReadOnlyList<Credential> credentials = CredentialManager.EnumerateCrendentials();
