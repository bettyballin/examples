...
namespace SandboxConsole
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var sessionKey = server.StartSession();
            Console.WriteLine($"Session Key: {BitConverter.ToString(sessionKey)}");

            var userKey = "myuserkey";
            var result = server.RollDice(userKey);
            Console.WriteLine($"Roll Result: {result.HmacMessage} - {result.Roll}");

            var serverKey = server.EndSession();
            Console.WriteLine($"Server Key: {BitConverter.ToString(serverKey)}");
        }
    }
}